...
def download_csv():
    ...
    # 应用品类筛选
    if category != "全部":
        filtered_df = category_df[category_df["PreferedOrderCat"] == category]
    else:
        filtered_df = category_df
    
    # 转换为CSV字符串
    output = StringIO()
    filtered_df.to_csv(output, index=False, encoding="utf-8-sig")
    output.seek(0)
    
    # 构建文件名
    filename = f"category_export_{category}.csv"
